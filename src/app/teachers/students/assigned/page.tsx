...
export default function AssignedStudents() {
  ...
  // Tick the time stat every minute
  useEffect(() => {
  ...
  const updateForm = (
    id: number,
    patch: Partial<{ link: string; token: string; expiry: string }>
  ) =>
    setForms((f) => ({
      ...f,
      [id]: {
  ...
  async function sendZoom(studentId: number) {
    try {
      ...
      if (!form?.link || !form?.token) {
        toast({ title: "Error", description: "Link and token are required.", variant: "destructive" });
        return;
      }
      ...
      toast({ title: "Error", description: e.message || "Failed to send Zoom link.", variant: "destructive" });
    } finally {
  ...
  async function saveAttendance(studentId: number) {
    try {
      ...
      if (!rec?.status) {
        toast({ title: "Error", description: "Attendance status is required.", variant: "destructive" });
        return;
      }
      setSending((s) => ({ ...s, [studentId]: true }));
      ...
      toast({ title: "Error", description: e.message || "Failed to save attendance.", variant: "destructive" });
    } finally {
      setSending((s) => ({ ...s, [studentId]: false }));
    }
  }
    ...
    return groups
      .map((g) => {
        const filteredStudents = g.students
          .filter((s) => {
            const matchesQuery = !q || safeIncludes(s.name, q) || safeIncludes(s.subject, q);
            const matchesPkg = filterPkg === "all" || safeIncludes(s.daypackages, filterPkg);
  ...
  const totalStudents = filteredGroups.reduce((acc, g) => acc + g.students.length, 0);
  const todayStudents = filteredGroups.reduce(
    (acc, g) => acc + g.students.filter((s) => s.occupied && s.occupied.length > 0).length,
    0
  );
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-8">
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-gray-200 p-6 sm:p-8 lg:p-10">
          <div className="flex flex-col lg:flex-row lg:items-center gap-8 mb-8">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-black mb-2">Student Management</h1>
                <p className="text-gray-600 text-base sm:text-lg lg:text-xl">Manage your assigned students, attendance, and class sessions</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:ml-auto">
              ...
              <div className="bg-gray-50 rounded-2xl p-4 text-center border border-gray-200">
                ...
                <div className="text-2xl font-bold text-black">{todayStudents}</div>
              </div>
              <div className="bg-gray-50 rounded-2xl p-4 text-center border border-gray-200">
                <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <div className="text-2xl font-bold text-black">{filteredGroups.length}</div>
              </div>
              <div className="bg-gray-50 rounded-2xl p-4 text-center border border-gray-200">
                <div className="flex items-center justify-center gap-2 mb-2">
          ...
          <div className="bg-gray-50 rounded-2xl p-6 border border-gray-200 sticky top-4 z-10">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-end">
              ...
              <div className="lg:col-span-2">
                <label className="block text-sm font-bold text-black mb-3">Quick Filter</label>
              ...
              <div className="lg:col-span-3 flex gap-3">
                ...
                >
                  {expAll ? <FiChevronUp className="mr-2 h-5 w-5" /> : <FiChevronDown className="mr-2 h-5 w-5" />}
                ...
                <Button
                  ...
                  aria-label="Refresh"
                >
                  <FiRefreshCcw className={`mr-2 h-5 w-5 ${loading ? "animate-spin" : ""}`} />
                  Refresh
                </Button>
              </div>
            </div>
        ...
        {error && (
          <div className="p-8 bg-white border border-red-200 rounded-3xl shadow-2xl flex items-center gap-6">
            ...
            <div>
              <h3 className="font-bold text-red-800 text-xl mb-2">Error Loading Students</h3>
        ...
        {!loading && !error && filteredGroups.length === 0 && (
          <div className="p-12 text-center bg-white rounded-3xl shadow-2xl border border-gray-200">
            ...
            <p className="text-gray-600 text-xl mb-6">No students match your current filters. Try adjusting your search criteria.</p>
            <Button onClick={refresh} className="bg-black text-white rounded-xl px-6 py-3 font-bold">Refresh</Button>
          </div>
        )}
        ...
        <div className="hidden lg:block space-y-8">
          {filteredGroups.map((g) => (
            <div key={g.group} className="rounded-3xl shadow-2xl border border-gray-200 bg-white overflow-hidden hover:shadow-3xl transition-all duration-300">
              <div className="p-8 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h2 className="font-bold text-2xl text-black mb-1">{g.group || "Unknown Package"}</h2>
                    <p className="text-gray-600 text-base">{g.students.length} student{g.students.length !== 1 ? "s" : ""} assigned</p>
                  </div>
                  <div className="bg-black text-white px-4 py-2 rounded-full font-bold">{g.students.length}</div>
                </div>
              </div>
                  ...
                  <table className="w-full">
                    <thead className="sticky top-0 bg-white">
                      <tr className="border-b-2 border-gray-200">
                        <th className="py-6 text-left font-bold text-black uppercase tracking-wider">Student Information</th>
                        <th className="py-6 text-left font-bold text-black uppercase tracking-wider">Subject & Contact</th>
                        <th className="py-6 text-left font-bold text-black uppercase tracking-wider">Schedule</th>
                        <th className="py-6 text-right font-bold text-black uppercase tracking-wider">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {g.students.map((s) => (
                        <tr key={s.id} className="border-b border-gray-100 hover:bg-gray-50 transition-all duration-200 group">
                          <td className="py-6">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <div className="font-bold text-black text-lg">{s.name || "Unnamed Student"}</div>
                          ...
                          <td className="py-6">
                            <div className="space-y-2">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="bg-gray-100 text-gray-800 px-3 py-1 rounded-full text-sm font-medium">{s.subject || "N/A"}</span>
                              </div>
                              {s.phone && (
                                <div className="flex items-center gap-2 text-sm text-gray-600">
                                  ...
                                  >
                                    <FiCopy className="inline h-3 w-3 mr-1" />Copy
                                  </button>
                                </div>
                              )}
                            </div>
                          ...
                          <td className="py-6">
                            <div className="flex flex-wrap items-center gap-2 text-gray-700">
                              ...
                              {s.occupied?.length ? (
                                s.occupied.map((o, i) => (
                                  <span key={i} className="px-2 py-1 bg-gray-100 rounded-full text-xs font-semibold text-gray-800">
                          ...
                          <td className="py-6 text-right">
                            <div className="flex gap-3 justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                              <Button onClick={() => setModal({ type: "zoom", studentId: s.id })} className="bg-black hover:bg-gray-800 text-white px-6 py-3 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all duration-200">
                                <FiLink2 className="h-4 w-4 mr-2" />
                                Send Zoom
                              </Button>
                              <Button onClick={() => setModal({ type: "attendance", studentId: s.id })} className="bg-gray-600 hover:bg-gray-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all duration-200">
        ...
        <div className="lg:hidden space-y-6">
          {filteredGroups.map((g) => (
            <div key={g.group} className="rounded-2xl shadow-xl border border-gray-200 bg-white overflow-hidden">
              <button className="w-full flex items-center justify-between p-6 hover:bg-gray-50 transition-colors touch-manipulation" onClick={() => toggleGroup(g.group)}>
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-black rounded-xl">
                    <FiBookOpen className="h-5 w-5 text-white" />
                  </div>
                  <div className="text-left">
                    <div className="font-bold text-black text-xl">{g.group || "Unknown Package"}</div>
                    <div className="text-gray-600 text-sm">{g.students.length} student{g.students.length !== 1 ? "s" : ""}</div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="bg-black text-white px-3 py-1 rounded-full font-bold text-sm">{g.students.length}</div>
                  {expanded[g.group] ? <FiChevronUp className="text-black h-6 w-6" /> : <FiChevronDown className="text-black h-6 w-6" />}
                </div>
              </button>
              ...
              {expanded[g.group] && (
                <div className="p-4 space-y-4 bg-gray-50">
                  {g.students.map((s) => (
                    <div key={s.id} className="p-6 rounded-2xl border border-gray-200 bg-white shadow-sm">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <Button onClick={() => setModal({ type: "zoom", studentId: s.id })} className="bg-black hover:bg-gray-800 text-white py-4 rounded-xl font-bold shadow-lg touch-manipulation">
                          <FiLink2 className="h-4 w-4 mr-2" />
                          Send Zoom Link
                        </Button>
                        <Button onClick={() => setModal({ type: "attendance", studentId: s.id })} className="bg-gray-600 hover:bg-gray-700 text-white py-4 rounded-xl font-bold shadow-lg touch-manipulation">
                          <FiCheck className="h-4 w-4 mr-2" />
                          Mark Attendance
                        </Button>
                      </div>
        ...
        {modal.type && modal.studentId !== null && (
          <>
            <div className="fixed inset-0 bg-black/60 z-40 animate-fade-in" onClick={() => setModal({ type: null, studentId: null })} />
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-100">
                  <div className="flex items-center gap-4">
                    <div className="p-3 bg-black rounded-2xl">
                      {modal.type === "zoom" ? <FiLink2 className="h-6 w-6 text-white" /> : <FiCheck className="h-6 w-6 text-white" />}
                    </div>
                    <div>
                      <h3 className="text-xl font-bold text-black">{modal.type === "zoom" ? "Send Zoom Link" : "Mark Attendance"}</h3>
                      <p className="text-sm text-gray-600">{modal.type === "zoom" ? "Share meeting details" : "Record student progress"}</p>
                    </div>
                  </div>
                  <button onClick={() => setModal({ type: null, studentId: null })} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-xl transition-colors" aria-label="Close">
                ...
                <div className="p-6 overflow-y-auto max-h-[70vh]">
                  {modal.type === "zoom" && (
                    <div className="space-y-6">
                      <div>
                        <label className="block text-sm font-bold text-black mb-3">Meeting Link</label>
                        <div className="flex gap-3">
                          <input
                            placeholder="https://zoom.us/j/..."
                            value={forms[modal.studentId]?.link || ""}
                            onChange={(e) => updateForm(modal.studentId!, { link: e.target.value })}
                          ...
                          />
                          <Button variant="outline" size="sm" onClick={() => handleCopy(forms[modal.studentId!]?.link || "")} className="px-4" aria-label="Copy meeting link">
                      ...
                      <div>
                        <label className="block text-sm font-bold text-black mb-3">Token</label>
                        <div className="flex gap-3">
                          <input
                            placeholder="Security token"
                            value={forms[modal.studentId]?.token || ""}
                            onChange={(e) => updateForm(modal.studentId!, { token: e.target.value })}
                          ...
                          />
                          <Button variant="outline" size="sm" onClick={() => updateForm(modal.studentId!, { token: genToken() })} className="px-4" aria-label="Generate token">
                            Gen
                          </Button>
                        </div>
                      </div>
                      ...
                      <div>
                        <label className="block text-sm font-bold text-black mb-3">Expires (Optional)</label>
                        <input
                          type="datetime-local"
                          value={forms[modal.studentId]?.expiry || ""}
                          onChange={(e) => updateForm(modal.studentId!, { expiry: e.target.value })}
                          className="w-full p-4 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-black focus:border-black bg-white text-gray-900 text-base"
                        />
                      </div>

                      <Button
                        disabled={!!sending[modal.studentId] || !forms[modal.studentId]?.link || !forms[modal.studentId]?.token}
                  ...
                  {modal.type === "attendance" && (
                    <div className="space-y-6">
                      <div>
                        <label className="block text-sm font-bold text-black mb-3">Status</label>
                        <div className="grid grid-cols-3 gap-3">
                          {(["present", "absent", "late"] as const).map((status) => (
                            <button
                              ...
                              aria-pressed={attend[modal.studentId!]?.status === status}
                            >
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-bold text-black mb-2">Level</label>
                          <input
                            placeholder="Juz 1"
                            value={attend[modal.studentId]?.level || ""}
                            onChange={(e) => updateAttend(modal.studentId!, { level: e.target.value })}
                        ...
                        <div>
                          <label className="block text-sm font-bold text-black mb-2">Surah</label>
                          <input
                            placeholder="Al-Fatiha"
                            value={attend[modal.studentId]?.surah || ""}
                            onChange={(e) => updateAttend(modal.studentId!, { surah: e.target.value })}
                            className="w-full p-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-black focus:border-black bg-white text-gray-900 placeholder-gray-500 text-base"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-bold text-black mb-2">Pages</label>
                          <input
                            type="number"
                            placeholder="5"
                            value={attend[modal.studentId]?.pages || ""}
                            onChange={(e) => updateAttend(modal.studentId!, { pages: e.target.value })}
                            className="w-full p-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-black focus:border-black bg-white text-gray-900 placeholder-gray-500 text-base"
                          />
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-black mb-2">Lesson</label>
                          <input
                            placeholder="Topic"
                            value={attend[modal.studentId]?.lesson || ""}
                            onChange={(e) => updateAttend(modal.studentId!, { lesson: e.target.value })}
                            className="w-full p-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-black focus:border-black bg-white text-gray-900 placeholder-gray-500 text-base"
                          />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) => updateAttend(modal.studentId!, { notes: e.target.value })}
                      ...
                      <Button
                        ...
                        disabled={!!sending[modal.studentId] || !attend[modal.studentId]?.status}
                        className="w-full bg-black hover:bg-gray-800 text-white font-bold py-4 rounded-xl shadow-lg disabled:opacity-50"
                      >
                        {sending[modal.studentId] ? (
                          <span className="flex items-center gap-2">
