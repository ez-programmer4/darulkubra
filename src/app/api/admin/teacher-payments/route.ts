...
export async function GET(req: NextRequest) {
  ...
  if (url.pathname.endsWith("/details") || url.searchParams.has("teacherId")) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const adminId = session.id ? parseInt(session.id) : undefined;
    ...
    await prisma.auditLog.create({
      data: {
        ...
        adminId: adminId ?? 1, // Use adminId instead of userId
        targetId: payment.id,
