...
export async function calculateTeacherSalary(teacherId: string, fromDate: Date, toDate: Date) {
      ...
      for (const link of studentLinks.values()) {
        ...
        if (latenessMinutes > excusedThreshold) {
          ...
          try {
            ...
            const isWaived = await isDeductionWaived(teacherId, scheduledTime, 'lateness');
            if (isWaived) {
              deduction = 0;
              tier = `${tier} (WAIVED)`;
            }
  ...
  if (currentStudents.length > 0) {
    for (let d = new Date(fromDate); d <= toDate; d.setDate(d.getDate() + 1)) {
      ...
      if (absenceResult.isAbsent) {
        ...
        try {
          const { isDeductionWaived } = await import('@/lib/deduction-waivers');
          const isWaived = await isDeductionWaived(teacherId, new Date(d), 'absence');
          if (isWaived) {
            dailyDeduction = 0;
